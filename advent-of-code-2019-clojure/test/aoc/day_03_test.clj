(ns aoc.day-03-test
  ...
  (:require [aoc.day-03 :refer :all]))

(deftest segment-split-no-op-test
  (testing "split segments no-op"
    (is (= [(segment [1 -1] (step 0 5))]
           (segment-split (segment [1 -1] (step 0 5)))))
    (is (= [(segment [-1 1] (step 1 5))]
           (segment-split (segment [-1 1] (step 1 5)))))
    (is (= [(segment [1 -1] (step 1 -5))]
           (segment-split (segment [1 -1] (step 1 -5)))))
    (is (= [(segment [-1 1] (step 0 -5))]
           (segment-split (segment [-1 1] (step 0 -5)))))
    (is (= [(segment [0 0] (step 0 -5))]
           (segment-split (segment [0 0] (step 0 -5)))))
    (is (= [(segment [0 0] (step 0 5))]
           (segment-split (segment [0 0] (step 0 5)))))
    (is (= [(segment [0 0] (step 1 -5))]
           (segment-split (segment [0 0] (step 1 -5)))))
    (is (= [(segment [0 0] (step 1 5))]
           (segment-split (segment [0 0] (step 1 5)))))))

(deftest segment-split-reverse-test
  (testing "split segments no-op"
    (is (= [(segment [-2 1] (step 0 -3))]
           (segment-split (segment [-5 1] (step 0 3)))))
    (is (= [(segment [1 -2] (step 1 -3))]
           (segment-split (segment [1 -5] (step 1 3)))))
    (is (= [(segment [2 1] (step 0 3))]
           (segment-split (segment [5 1] (step 0 -3)))))
    (is (= [(segment [1 2] (step 1 3))]
           (segment-split (segment [1 5] (step 1 -3)))))))

(deftest segment-split-do-test
  (testing "split segments no-op"
    (is (= [(segment [0 1] (step 0 -5)) (segment [0 1] (step 0 3))]
           (segment-split (segment [-5 1] (step 0 8)))))
    (is (= [(segment [0 1] (step 0 5)) (segment [0 1] (step 0 -3))]
           (segment-split (segment [5 1] (step 0 -8)))))
    (is (= [(segment [-5 0] (step 1 -1)) (segment [-5 0] (step 1 7))]
           (segment-split (segment [-5 -1] (step 1 8)))))
    (is (= [(segment [5 0] (step 1 1)) (segment [5 0] (step 1 -7))]
           (segment-split (segment [5 1] (step 1 -8)))))))
...
(deftest find-closest-collision-test
  (testing "find closest collision for advent of code 2019 day 3 p1"
    (is (= [[258 0] 0 1]
