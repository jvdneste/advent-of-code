...
(def program-values (str/split program-text #","))

(defn program-terminates [program position]
  (= (nth program position) 99))

(defn program-eval-at-position [program position]
  (let [opcode (nth program position) arg1 (nth program (+ position 1)) arg2 (nth program (+ position 2)) result-position (nth program (+ position 3))]
    (case opcode
...
(def result (program-eval-from-position-to-end program-values 0))
