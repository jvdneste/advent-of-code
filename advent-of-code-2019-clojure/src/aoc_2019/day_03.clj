...
(defrecord Translation [axis value])

(defn create-translation [direction distance]
  (case direction
    "R" (Translation. 0 distance)
    "L" (Translation. 0 (- distance))
    "U" (Translation. 1 distance)
    "D" (Translation. 1 (- distance))))

(defn parse-translation [text]
  (create-translation
    (subs text 0 1)
    (Integer. (subs text 1))))

(defn parse-wire [text]
  (->>
    (str/split text #",")
    (map parse-translation)))

(defn translate [point translation]
  (let [{axis :axis value :value} translation]
    (assoc point axis (+ (nth point axis) value))))

(def center [0 0])

(defrecord Segment [point translation])
