...
(defrecord Step [direction distance])

(defn parse-step [text]
  (Step.
    (subs text 0 1)
    (Integer. (subs text 1))))
...
(defrecord Point [x y] Translatable
  (move [this step]
    (case (:direction step)
      "U" (up this (:distance step))
      "D" (down this (:distance step))
      "L" (left this (:distance step))
      "R" (right this (:distance step))))
  (up [this direction]
    (Point. (+ x direction) y))
  (down [this direction]
    (Point. (- x direction) y))
